...
for (let i = 0; i < graduates.length; i++) {
  const uni = graduates[i].university;
...
const contactInfo = [];
for (let i = 0; i < graduates.length; i++) {
  const first = graduates[i].first_name;
  const email = graduates[i].email;
  contactInfo.push(`${first} ${email}`)
}
...
const uni = [];
for (let i = 0; i < graduates.length; i++) {
  const element = (graduates[i].university).split();
  const strings = element.toString();
 if (strings.includes('Uni') === true) {
      uni.push(strings);
    }
}
console.log(uni);
...
const low = zooAnimals.filter(element => {
...
const total = zooAnimals.reduce((acc, currentValue) => {
  return {population: acc.population + currentValue.population};
})
console.log(total);
